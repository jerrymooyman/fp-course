...
product ::
  List Int
  -> Int
product l = foldLeft (\acc i -> acc * i) 1 l
-- product' = foldLeft (\acc i -> acc * i) 1
...
flatten ::
  List (List a)
  -> List a
flatten l = foldRight (\i acc -> i ++ acc) Nil l
...
-- seqOptional' l = foldRight (\i acc -> bindOptional i acc ) Empty l
...
-- find' f = foldRight (\i acc -> if f i then Full i else acc) Empty
-- find'' f l =
--   case filter f l of
--     Nil  -> Empty
--     h:._ -> Full h
-- find''' l = foldRight (const . Full) Empty . filter l
